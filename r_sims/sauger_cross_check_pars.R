##################################################
## sauger_cross_check_trios.R by JPJ 15 vii 24
##################################################
...
for (g in 1:length(nloci)) {
	for (h in 1:length(wild_samp)) {
		truth <- read.delim(paste0("true_parents_", nloci, "_", wild_samp, ".txt"), header=FALSE, sep=" ") ## true parental relationships
		pars <- read.delim(paste0("pars_", nloci, "_", wild_samp, ".txt"), header=TRUE, sep=" ")	 ## parental relationships from sequoia
		for (i in 1:dim(pars)[1]) {
			if (grepl("f1h", pars[i,2])==TRUE) { ## if offspring is from f1h generation
