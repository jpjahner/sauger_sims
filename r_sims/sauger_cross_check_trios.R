...
for (g in 1:length(nloci)) {
	for (h in 1:length(wild_samp)) {
		...
		for (i in 1:dim(trios)[1]) {
			if (trios[i,2] %in% all_parents[all_parents[,1]==trios[i,1],2:3]==TRUE) { right_count <- right_count+1 }
			else                                                                    { wrong_count <- wrong_count+1 }
			if (trios[i,3] %in% all_parents[all_parents[,1]==trios[i,1],2:3]==TRUE) { right_count <- right_count+1 }
			else                                                                    { wrong_count <- wrong_count+1 }
		}
