...
par(mar=c(5,5,1,1))
hist(freqs[,5], breaks=100, xlab="Allele frequency", ylab="Number of loci", main="", cex.axis=1.25, cex.lab=1.5, las=1); box(lwd=1.5)
par(new=TRUE)
...
mtext(bquote(alpha==.(round(beta_fit$alpha,2))), cex=1.25, adj=0.02, line=-1.15)
mtext(bquote(beta==.(round(beta_fit$beta,2))), cex=1.25, adj=0.02, line=-2.4)
dev.off()
